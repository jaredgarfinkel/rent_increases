...
result2 = result |> 
  mutate(word_number = row_number(),
         ...
         col1 = case_when(rent_number %in% c(1:length(word_number)) & !is.na(lead(date_start,1)) ~ lead(date_start, 1)),
         col2 = case_when(rent_number %in% c(1:length(word_number)) & !is.na(lead(date_end, 3)) ~ lead(date_end, 3)),
         col3 = case_when(rent_number %in% c(1:length(word_number)) & !is.na(lead(one_year_raise, 4)) ~ lead(one_year_raise, 4)),
         col4 = case_when(rent_number %in% c(1:length(word_number)) & !is.na(lead(two_year_raise, 5)) ~ as.numeric(lead(two_year_raise, 5)))) |>
